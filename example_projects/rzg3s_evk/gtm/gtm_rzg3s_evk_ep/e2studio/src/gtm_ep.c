...
fsp_err_t gtm_init(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** R_GTM_Open API for timer 2 failed. **\r\nRestart the Application\r\n");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        {
            APP_ERR_PRINT("** R_GTM_Open API for timer 1 failed. **\r\nRestart the Application\r\n");
        }
...
fsp_err_t gtm_start_oneshot_timer(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\n R_GTM_Start API for timer 2 failed **\r\n");
    }
    return err;
}
...
void one_shot_timer_callback(timer_callback_args_t *p_args)
{
    ...
    else
    {
        if (TIMER_STATE_COUNTING != periodic_timer_status.state)
        {
            ...
            {
                g_error_flag = SET_FLAG;
                APP_ERR_PRINT("** R_GTM_Start API for timer 1 failed in Periodic mode **\r\n");
            }
            else
            {
                /* Set the flag since Timer 1 is started in periodic mode */
                g_periodic_timer_flag = SET_FLAG;
            }
        }
        else
        {
...
void periodic_timer_callback(timer_callback_args_t *p_args)
{
    ...
    fsp_err_t err = R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t) g_bsp_leds.p_leds[0],
                                      led_level); /* [0] for Pmod0 pin 7 */
