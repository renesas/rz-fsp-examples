...
/* Read Device signature/ID from sensor */
static fsp_err_t get_device_id (uint8_t * dev_id);

/* Validate i2c receive/transmit based on required write read operation */
...
fsp_err_t init_sensor(void)
{
    ...
    if ((FSP_SUCCESS == err) && (DEVICE_SIGNATURE == device_id))
    {
        ...
        if (FSP_SUCCESS == err)
        {
            ...
            {
                APP_ERR_PRINT("** validate_i2c_event failed **\r\n");
            }
        }
 ...
 *  @param[IN]   xyz_data                     Array pointer to get the sensor data.
 *  @retval      FSP_SUCCESS                  Upon successful open and start of timer.
