...
void hal_entry(void)
{
    ...
    uint8_t user_input[BUFF_SIZE] = {NULL_CHAR};
    ...
    /* On successful operations on ELC and GPT modules, user is asked enter any key as input */
    APP_PRINT("\r\nEnter any key as input. GPT counter is triggered and counter value is displayed"
                " on Tera Term\r\n");
    ...
    while (true)
    {
        if (APP_CHECK_DATA)
        {
            memset(&user_input[0], NULL_CHAR, BUFF_SIZE);
            APP_READ(user_input);
            ...
            * without calling timer start API. */
            ...
            /* Display counter value on Tera Term */
