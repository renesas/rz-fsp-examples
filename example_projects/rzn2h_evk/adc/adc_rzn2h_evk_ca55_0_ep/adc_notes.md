...
RZ MPUs based on Renesas FSP. An Tera Term input command triggers the ADC scan to read the analog data of adc channel and outputs 
the converted data onto the Tera Term output.The project initializes the ADC in single scan or continuous scan mode based on user 
...
scan mode) using Tera Term by sending commands. User provide ADC channel input voltage from 0V to 1.8V with a variable 
...
act accordingly. Result and ADC status are displayed on the Tera Term.
...
### Hardware
* 1x RZ/N2H Evaluation kit
* 1x USB A to USB Micro B Cable
* 1x Potentiometer on RZ/N2H Evaluation kit
...
|-------------|-----------------------------------------------|-----------------------------------------------|
|ADC | Driver for the ADC peripheral to demonstrate basic functionalities of ADC, including the Window Compare Mode on Renesas RZ MPUs | adc|
...
| configuration.xml -> HAL/Common -> Properties > Settings > Property > Module g_adc ADC (r_adc) >Input > Window Compare > Window A > Lower Reference | 0 | 1241 | To configure Lower reference for Window Compare Mode in Window A for 12bit ADCs |
| configuration.xml -> HAL/Common -> Properties > Settings > Property > Module g_adc ADC (r_adc) >Input > Window Compare > Window A > Upper Reference | 0 | 3103 | To configure Upper reference for Window Compare Mode in Window A for 12bit ADCs |
...
| configuration.xml -> HAL/Common -> Properties > Settings > Property > Module g_adc ADC (r_adc) >Interrupts > Scan End Interrupt Priority | Disabled | Priority 7 | To assign interrupt priority for scan end event |
| configuration.xml -> HAL/Common -> Properties > Settings > Property > Module g_adc ADC (r_adc) >Interrupts > Window Compare A Interrupt Priority | Disabled | Priority 7 | To assign interrupt priority for window compare event of Window A |
...
## Verifying operation
1. Import, generate and build adc EP in e2studio.
   Before running the example project, make sure hardware connections are done.
2. Download adc EP to one Renesas RZ MPU Evaluation kit and run the project.
...
5. Also user can supply input voltage to ADC scan channel pin and verify result on Tera Term.
6. If the ADC value is outside the window compare, it status is shown in the Tera Term.
   The Boundary values of ADC Window Compare Mode are
      - Lower Reference is 0.54V
      - Upper Reference is 1.36V
      - Reference Voltage is 1.8V
...
## ADC Value calculation
   ...
   1) If the Analog input voltage is 1.5V
    	For 12 bit ADC
		Digital Output = (2^12 * 1.5)/1.8
		               = (4096 * 1.5)/1.8
					   = 6144/1.8
		Digital Output = 3413
		
   
   Below images showcases the ADC output on Tera Term:

   ![ADC_Banner](images/ADC_Banner.png "Banner Info")
 
   ![ADC_Window_Lower_Limit](images/ADC_Window_Lower_Limit.png "Tera Term output - ADC_Window_Lower_Limit")
 
   ![ADC_values_within_Window_Limits](images/ADC_values_within_Window_Limits.png "Tera Term output - ADC_values_within_Window_Limits")
 
   ![ADC_Window_Upper_Limit](images/ADC_Window_Upper_Limit.png "Tera Term output - ADC_Window_Upper_Limit")
