...
This example project demonstrates Ethernet communication on RZ/N2H using FreeRTOS+TCP based on Renesas FSP.
...
### g_ether_phy0 Ethernet
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
| :-------------------------------------: | :---------------: | :------------: | :--------: |
|  Module > Speed | 10/100/1000M | 100M | The PHY speed is fixed to 100 Mbps to match the MII interface limitation and ensure stable operation on ETH0. |

### g_ether_phy0 Ethernet -> g_ether_selector0
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
| :-------------------------------------: | :---------------: | :------------: | :--------: |
|  Module > Interface Type | RGMII | MII | ETH0 is configured to use the MII interface to represent a standard 10/100 Mbps Ethernet use case. |

### g_ether_phy1 Ethernet
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
| :-------------------------------------: | :---------------: | :------------: | :--------: |
|  Module > PHY-LSI Address | 0 | 1 | The PHY address is set to match the physical hardware connection of Ethernet Port 1 on the board. |
|  Module > Speed | 10/100/1000M | 100M | The PHY speed is limited to 100 Mbps to comply with the MII interface used by ETH1. |

### g_ether_phy1 Ethernet -> g_ether_selector1
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
| :-------------------------------------: | :---------------: | :------------: | :--------: |
|  Module > Interface Type | RGMII | MII | ETH1 is configured as an MII port to provide an additional 10/100 Mbps Ethernet interface for redundancy or secondary network use cases. |

### g_ether_phy2 Ethernet
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
| :-------------------------------------: | :---------------: | :------------: | :--------: |
|  Module > PHY-LSI Address | 0 | 2 | The PHY address is configured to correctly correspond to the physical hardware connection of Ethernet Port 2 on the board. |
...
9. Repeat the same connection and verification steps by moving the Ethernet cable to ETH0, ETH1, and ETH2 one at a time to confirm operation on all monitored ports.
   ...
   Below images showcases the FreeRTOSplusTCP output on JLinkRTTViewer and Command Prompt:
...
11. Configure a UDP packet with the following settings:
   ...
   - Port: `10000`
   - Payload: any ASCII text (for example, Hello_UDP_RZN2H)
...
14. Verify on J-Link RTTViewer that the application displays UDP receive/transmit logs, including the sender IP address, payload length, and echoed response status.

   <img src="images/RTT_Viewer_UDP_echo_result.png" alt="RTT_Viewer_UDP_echo_result" title="Packet Sender UDP Echo results on RTT Viewer" width="800"/>
...
15. Configure a TCP packet with the following settings:
   ...
   - Address: `192.168.0.100`
   - Port: `10000`
   - Payload: any ASCII text (for example, Hello_TCP_RZN2H)
...
18. Verify on J-Link RTTViewer that the application displays TCP receive/transmit logs, including the sender IP address, payload length, echoed response status and Client disconnection.

   <img src="images/RTT_Viewer_TCP_echo_result.png" alt="RTT_Viewer_TCP_echo_result" title="Packet Sender TCP Echo results on RTT Viewer" width="800"/>
