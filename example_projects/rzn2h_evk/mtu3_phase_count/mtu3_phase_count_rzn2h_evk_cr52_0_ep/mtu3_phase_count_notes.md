...
It counts pulses from a two-phase encoder (A phase, B phase) and provides J-Link RTT Viewer commands to get, set, and reset the count value.
...
Result and MTU3 status are displayed on the J-Link RTT Viewer.
...
5. Verify phase count direction by rotating the encoder shaft:
   ...
   The images below showcases the output on J-Link RTT Viewer
