...
fsp_err_t spi_write_and_read(void)
{
    ...
    g_master_event_flag = (spi_event_t) RESET_VALUE;  /* Resetting master_event flag. */
    g_slave_event_flag  = (spi_event_t) RESET_VALUE;  /* Resetting slave_event flag. */
...
fsp_err_t spi_write_read(void)
{
    ...
    err = R_RSPI_WriteRead(&g_spi_slave_ctrl, g_slave_tx_buff, g_slave_rx_buff,
                           num_bytes_master, SPI_BIT_WIDTH_32_BITS);

    /* Error handle. */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_RSPI_WriteRead API failed");
        return err;
    }

    /* Master send data to Slave and receive data from Slave. */
    err = R_RSPI_WriteRead(&g_spi_master_ctrl, g_master_tx_buff, g_master_rx_buff,
                           num_bytes_master, SPI_BIT_WIDTH_32_BITS);
