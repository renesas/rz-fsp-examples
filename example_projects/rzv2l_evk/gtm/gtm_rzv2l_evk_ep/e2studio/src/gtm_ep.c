...
fsp_err_t gtm_init(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_GTM_Open for channel 2 API failed.\r\nRestart the Application\r\n");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        {
            APP_ERR_PRINT("\r\nR_GTM_Close for channel 2 API failed.\r\nRestart the Application\r\n");
        }
        APP_ERR_PRINT("\r\nR_GTM_Open for channel 1 API failed.\r\nRestart the Application\r\n");
    }
    return err;
}
...
fsp_err_t gtm_start_oneshot_timer(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nR_GTM_Start for channel 2 API failed.\r\n");
    }
    return err;
}
...
void one_shot_timer_callback(timer_callback_args_t *p_args)
{
    ...
    else
    {
        if (TIMER_STATE_COUNTING != periodic_timer_status.state)
        {
            ...
            {
                g_error_flag = SET_FLAG;
                APP_ERR_PRINT("\r\nR_GTM_Start for channel 1 API failed.\r\n");
            }
            else
            {
...
void periodic_timer_callback(timer_callback_args_t *p_args)
{
    ...
    /* Change LED state connected to Pmod0 pin 9. */
    fsp_err_t err = R_IOPORT_PinWrite(&g_ioport_ctrl,
                                       (bsp_io_port_pin_t) g_bsp_leds.p_leds[0],
                                       led_level);
