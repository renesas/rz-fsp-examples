 ...
 * Description  : Initialize GTM timer and periodically send messages to the queue; on suspend request, stop/close
...
void sender_task_entry(void *pvParameters)
{
    ...
    while (true)
    {
         ...
         * continue sending messages to Message Queue at 500 miliseconds periodically. */
        if (true != b_suspend_msqQ_task)
        {
            ...
            if (pdPASS == xQueueSendToBack(g_queue, &s_message_to_task, Queue_wait_ticks))
            {
                ...
                APP_PRINT("\r\nSender_Task : Going on delay for 500 miliseconds");

                /* Places the task into the Blocked state. The task
                 * will execute every 500 miliseconds once */
                vTaskDelayUntil(&last_execution_time, WAIT_TIME);

                APP_PRINT("\r\nSender_Task : After delay of 500 miliseconds\r\n");
            }
        }
        else
        {
 ...
 * Description  : User defined GTM IRQ callback for Message Queue.
 * Argument     : p_args
 * Return Value : None
 **********************************************************************************************************************/
