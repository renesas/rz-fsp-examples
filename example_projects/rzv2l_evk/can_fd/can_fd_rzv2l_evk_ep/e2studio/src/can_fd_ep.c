...
{
    /* Conversion to 16-bit unsigned integer value */
...
static void can_write_operation(canfd_instance_ctrl_t * p_api_ctrl, can_frame_t * p_frame);
static void can_fd_data_update(void);
static void can_data_check_operation(void);

/***********************************************************************************************************************
 * @brief       This function trasmits data after receiving user input and it performs subsequent operations.
 * @param[in]   None
 * @return      None
 **********************************************************************************************************************/
 ...
 * @param[in]   p_api_ctrl                Canfd instance control structure
...
static void can_write_operation(canfd_instance_ctrl_t * p_api_ctrl, can_frame_t * p_frame)
{
    ...
    err = R_CANFD_Write(p_api_ctrl, CAN_MAILBOX_NUMBER_0, p_frame);
...
static void can_data_check_operation(void)
{
    ...
    if (RESET_VALUE == strncmp((char*)&g_canfd_ch1_rx_frame.data[ZERO],
                                 (char*)&tx_data[ZERO], CAN_CLASSIC_FRAME_DATA_BYTES))
    {
        ...
        APP_PRINT("\nCAN transmission after receive is successful."
                  " Sent back the ACK using classic CAN frame");
    }
    else if (RESET_VALUE == strncmp((char*)&g_canfd_ch0_rx_frame.data[ZERO],
                                      (char*)&rx_data[ZERO], CAN_CLASSIC_FRAME_DATA_BYTES))
    {
        /* Cleaning receive frame. */
        ...
        {
            g_canfd_ch0_tx_frame.data[j] = (uint8_t)(j + 1); /* Conversion to 8-bit unsigned integer value */
        }
    ...
    /* Acknowledging for second transmission. */
                                      ...
                                      (char*)&tx_fd_data[ZERO], CAN_FD_DATA_LENGTH_CODE))
    {
        /* Cleaning receive frame. */
        ...
        for (uint16_t j = 0; j < SIZE_64; j++)
        {
            g_canfd_ch1_tx_frame.data[j] = (uint8_t)(j + 5); /* Conversion to 8-bit unsigned integer value. */
        }
    ...
    /* Acknowledgement for second transmission */
                                      ...
                                      (char*)&rx_fd_data[ZERO], CAN_FD_DATA_LENGTH_CODE))
    {
        /* Cleaning receive frame. */
        memset(&g_canfd_ch0_rx_frame.data[ZERO], NULL_CHAR, CAN_FD_DATA_LENGTH_CODE);
...
static void can_fd_data_update(void)
{
    ...
    {
        tx_fd_data[i] = (uint8_t)(i + 1); /* Conversion to 8-bit unsigned integer value */
    }
    for (uint16_t j = 0; j < SIZE_64; j++)
    {
        rx_fd_data[j] = (uint8_t)(j + 5); /* Conversion to 8-bit unsigned integer value */
    }
}
...
/***********************************************************************************************************************
 * @brief      CANFD callback function for channel 0.
 * @param[in]  p_args
 * @retval     None
 **********************************************************************************************************************/
...
/***********************************************************************************************************************
 * @brief      CANFD callback function for channel 1.
 * @param[in]  p_args
 * @retval     None
 **********************************************************************************************************************/
...
void canfd_deinit(void)
{
    ...
    /* Close CANFD channel 0 */
    err = R_CANFD_Close(&g_canfd_ch0_ctrl);
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\n**R_CANFD_Close channel 0 API failed**");
    }

    /* Close CANFD channel 1 */
    err = R_CANFD_Close(&g_canfd_ch1_ctrl);
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\n**R_CANFD_Close channel 1 API failed**");
    }
}
