...
void hal_entry(void)
{
    ...
    while (true)
    {
        if (APP_CHECK_DATA)
        {
            ...
            /* Enter "HIGH" from P05_6(GPIO) to P18_1(ELC Single Port 0) to generate the event. */
            /* When P05_6 and P18_1 are connected on the board, the timer start counting */
            /*  without calling timer start API. */
            R_BSP_PinAccessEnable();
            R_BSP_PinToggle(BSP_IO_REGION_SAFE, BSP_IO_PORT_05_PIN_6);
            ...
            /* Verify the timer_value not equal 0 */
            if (0 == timer_value)
            {
                elc_deinit();
                gpt_deinit(&g_timer0_ctrl);
