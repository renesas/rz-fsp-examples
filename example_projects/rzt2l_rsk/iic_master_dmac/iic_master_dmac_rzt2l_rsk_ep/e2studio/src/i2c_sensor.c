...
fsp_err_t init_sensor(void)
{
    ...
    /* Verify Transfer Settings Are Null */
    if(g_transfer0_ctrl.p_cfg->p_info->p_src != NULL || g_transfer0_ctrl.p_cfg->p_info->p_dest != NULL)
    {
        APP_ERR_PRINT("** DMAC transfer0 settings init is not NULL **\r\n");
        return FSP_ERR_INVALID_STATE;
    }
    if(g_transfer1_ctrl.p_cfg->p_info->p_src != NULL || g_transfer1_ctrl.p_cfg->p_info->p_dest != NULL)
    {
        APP_ERR_PRINT("** DMAC transfer1 settings init is not NULL ** \r\n");
        return FSP_ERR_INVALID_STATE;
    }
    ...
    if ((FSP_SUCCESS == err) && (DEVICE_SIGNATURE == device_id))
    {
        ...
        err = R_IIC_MASTER_Write(&g_i2c_master_ctrl, g_measure_enable_payload, TWO_BYTE, false);
        if (FSP_SUCCESS == err)
        {
...
fsp_err_t read_sensor_data(uint8_t * p_xyz_data)
{
    ...
    err = R_IIC_MASTER_Write(&g_i2c_master_ctrl, &g_internal_reg, ONE_BYTE, true);
    ...
    else
    {
        ...
        if (FSP_SUCCESS == err)
        {
            err = R_IIC_MASTER_Read(&g_i2c_master_ctrl, &g_adxl_data[0], DATA_REGISTERS, false);
        ...
        {
            *(p_xyz_data + itr) = g_adxl_data[itr];
        }
    }

    /* Verify that DMAC transfer and receive source and destination are valid after write operation */
    if(g_transfer0_ctrl.p_cfg->p_info->p_src == NULL || g_transfer0_ctrl.p_cfg->p_info->p_dest == NULL)
    {
        APP_ERR_PRINT("** DMAC transfer0 not active ** \r\n");
        return FSP_ERR_INVALID_STATE;
    }
    if(g_transfer1_ctrl.p_cfg->p_info->p_src == NULL || g_transfer1_ctrl.p_cfg->p_info->p_dest == NULL)
    {
        APP_ERR_PRINT("** DMAC transfer1 not active ** \r\n");
        return FSP_ERR_INVALID_STATE;
    }
...
static fsp_err_t get_device_id(uint8_t *dev_id)
{
    ...
    err = R_IIC_MASTER_Write(&g_i2c_master_ctrl, &g_reg_address, ONE_BYTE, true);
    ...
    /* Verify that DMAC transfer source and destination are valid after write operation */
    if(g_transfer0_ctrl.p_cfg->p_info->p_src == NULL || g_transfer0_ctrl.p_cfg->p_info->p_dest == NULL)
    {
        APP_ERR_PRINT("** DMAC transfer0 not active ** \r\n");
        return FSP_ERR_INVALID_STATE;
...
static fsp_err_t validate_i2c_event(void)
{
    ...
    {
        s_i2c_event = (i2c_master_event_t)RESET_VALUE;  /* Make sure this is always Reset before return */
        return FSP_SUCCESS;
    }
