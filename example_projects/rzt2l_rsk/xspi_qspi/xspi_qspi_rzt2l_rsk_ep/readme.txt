...
1. Project Overview:
    After debug connection to RSK board, this sample program initializes the xSPI. 
    Performs to set the Serial Flash to Quad mode, and executes the PageProgram command 
    and Read command. Then verify the data of PageProgram and Read. Finally read the data 
    from Serial Flash by memory-mapped mode(direct-read from memory mapped address) and 
    verify the data, status is printed to the RTT Viewer.
...
4. Hardware Configurations:
    ...
    Hardware Connection:
        ...
        3. *QSPI Flash is connected to xSPI1 unit, and the following settings are required.
        SW8.9 : ON
        CN33 : 1-2 short
