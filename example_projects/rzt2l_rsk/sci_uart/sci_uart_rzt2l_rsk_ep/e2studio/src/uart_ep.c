...
fsp_err_t uart_print_user_msg(uint8_t *p_msg)
{
    ...
    uint8_t * p_temp_ptr = (uint8_t *)p_msg;  /* cast to char * type */

    /* Calculate length of message received */
    msg_len = ((uint8_t)(strlen((char *)p_temp_ptr)));  /* cast to char * type */
        ...
        switch (p_args->data)
        {
            ...
            default:
            {
                if (s_counter_var < DATA_LENGTH)
                {
                    ...
                    s_temp_buffer[s_counter_var++] = (uint8_t)p_args->data;  /* cast to uint8_t type */
