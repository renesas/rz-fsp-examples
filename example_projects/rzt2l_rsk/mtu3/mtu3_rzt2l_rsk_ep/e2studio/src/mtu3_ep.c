...
fsp_err_t init_mtu3(void)
{
    ...
    err = R_MTU3_PeriodSet(&g_timer0_ctrl, MTU3_2_5MS_PCLKH_16);
    if (FSP_SUCCESS != err)
    {
    ...
    err = R_MTU3_DutyCycleSet(&g_timer0_ctrl, s_mtu3_tgr_data[DEFAULT_TIMER_SETTING], UNUSED_PARAM);
    if (FSP_SUCCESS != err)
    {
...
void r_main_mtu_change_timer(void)
{
    ...
    /* Update duty cycle */
    err = R_MTU3_DutyCycleSet(&g_timer0_ctrl, s_mtu3_tgr_data[g_duty_cnt], UNUSED_PARAM);
    if (FSP_SUCCESS != err)
    {
...
/***********************************************************************************************************************
 * End of function: r_main_mtu_change_timer
 **********************************************************************************************************************/
