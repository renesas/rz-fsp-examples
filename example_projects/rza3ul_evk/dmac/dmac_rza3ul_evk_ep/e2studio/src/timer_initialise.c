...
fsp_err_t mtu3_timer_init(void)
{
    ...
    {
        APP_ERR_PRINT("** R_MTU3_Open API failed. Necessary cleanup is done. **\r\n");
        return fsp_err;
    }
    ...
    if (FSP_SUCCESS != fsp_err)
    {
        ...
        /* Failure message */
        APP_ERR_PRINT("** R_MTU3_Start API failed. Necessary cleanup is done. **\r\n");
    }
    return fsp_err;
}
...
fsp_err_t gtm_timer_init(void)
{
    ...
    if (FSP_SUCCESS != fsp_err)
    {
        ...
        /* Failure message */
        APP_ERR_PRINT("** R_GTM_Start API failed. Necessary cleanup is done. **\r\n");
    }
    return fsp_err;
}
