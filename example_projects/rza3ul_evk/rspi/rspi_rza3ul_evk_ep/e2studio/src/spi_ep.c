...
static uint32_t s_master_tx_buff[BUFF_LEN] __attribute__((section("UNCACHED_BSS")));   /* Master Transmit Buffer */
static uint32_t s_master_rx_buff[BUFF_LEN] __attribute__((section("UNCACHED_BSS")));   /* Master Receive Buffer */
static uint32_t s_slave_tx_buff[BUFF_LEN] __attribute__((section("UNCACHED_BSS")));    /* Slave Transmit Buffer */
static uint32_t s_slave_rx_buff[BUFF_LEN] __attribute__((section("UNCACHED_BSS")));    /* Slave Receive Buffer */
...
fsp_err_t spi_write_and_read(void)
{
    ...
    while (BYTES_RECEIVED_ZERO == num_bytes)
    {
        if (APP_CHECK_DATA)
        {
            APP_READ((unsigned char*)s_master_tx_buff);
            APP_PRINT("\r\n");
            APP_PRINT("Input data: %s\r\n", (char *)s_master_tx_buff);

            num_bytes =  (uint32_t)strlen((char *)s_master_tx_buff);
    ...
    err = R_RSPI_Read(&g_spi_slave_ctrl, s_slave_rx_buff, num_bytes, SPI_BIT_WIDTH_32_BITS);

    /* Error handle */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Slave R_RSPI_Read API failed\r\n");
        return err;
    }

    /* Master send data to Slave */
    err = R_RSPI_Write(&g_spi_master_ctrl, s_master_tx_buff, num_bytes, SPI_BIT_WIDTH_32_BITS);

    /* Error handle */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Master R_RSPI_Write API failed\r\n");
        return err;
    }
    ...
    s_master_event_flag = (spi_event_t) RESET_VALUE;  /* Reset master_event flag */
    s_slave_event_flag  = (spi_event_t) RESET_VALUE;  /* Reset slave_event flag */

    /* Display Master to Slave transmission */
    APP_PRINT("Master transmitted user input data to Slave\r\n");

    /* Slave send data to Master */
    err = R_RSPI_Write(&g_spi_slave_ctrl, s_slave_rx_buff, num_bytes, SPI_BIT_WIDTH_32_BITS);

    /* Error handle */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Slave R_RSPI_Write API failed\r\n");
        return err;
    }

    /* Master receive data from Slave */
    err = R_RSPI_Read(&g_spi_master_ctrl, s_master_rx_buff, num_bytes, SPI_BIT_WIDTH_32_BITS);

    /* Error handle */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Master R_RSPI_Read API failed\r\n");
        return err;
    }
...
fsp_err_t spi_write_read(void)
{
    ...
    while (BYTES_RECEIVED_ZERO == num_bytes_master)
    {
        if (APP_CHECK_DATA)
        {
            ...
            APP_PRINT("\r\n");
            APP_PRINT("Input data: %s\r\n", (char *)s_master_tx_buff);
            num_bytes_master = (uint32_t)strlen((char *)s_master_tx_buff);
    ...
    while (BYTES_RECEIVED_ZERO == num_bytes_slave)
    {
        if (APP_CHECK_DATA)
        {
            APP_READ((unsigned char*)s_slave_tx_buff);
            APP_PRINT("\r\n");
            APP_PRINT("Input data: %s\r\n", (char *)s_slave_tx_buff);
            num_bytes_slave = (uint32_t)strlen((char *)s_slave_tx_buff);
    ...
    err = R_RSPI_WriteRead(&g_spi_slave_ctrl, s_slave_tx_buff, s_slave_rx_buff, num_bytes_master, SPI_BIT_WIDTH_32_BITS);

    /* Error handle */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Slave R_RSPI_WriteRead API failed\r\n");
        return err;
    }

    /* Master send data to Slave and receive data from Slave */
    err = R_RSPI_WriteRead(&g_spi_master_ctrl, s_master_tx_buff, s_master_rx_buff, num_bytes_master, SPI_BIT_WIDTH_32_BITS);

    /* Error handle */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Master R_RSPI_WriteRead API failed\r\n");
        return err;
    }
...
fsp_err_t spi_exit_demo(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Master R_RSPI_Close API failed\r\n");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("Slave R_RSPI_Close API failed\r\n");
    }
    return err;
}
