...
void hal_entry(void)
{
    ...
    APP_PRINT("[CPU0] Open SHARED_MEMORY Driver...\r\n");
    err = R_SHARED_MEMORY_Open(&g_shared_memory0_ctrl, &g_shared_memory0_cfg);
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("[CPU0] Open failed: %d\r\n", err);
        APP_ERR_TRAP(err);
    }
    APP_PRINT("[CPU0] Open: OK\r\n");

    /* Wait until the driver of CPU1 opens. */
    APP_PRINT("[CPU0] Waiting for READY_TO_WRITE ...\r\n");
    do
    {
        err = R_SHARED_MEMORY_StatusGet(&g_shared_memory0_ctrl, &status);
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("[CPU0] StatusGet failed: %d\r\n", err);
            APP_ERR_TRAP(err);
        }
    }
    ...
    offset = 0;
    APP_PRINT("[CPU0] Write(offset=%lu, len=%lu) -> IRQ to CPU1\r\n",
    ...
    {
        APP_ERR_PRINT("[CPU0] Compare: MISMATCH\r\n");
        APP_ERR_TRAP(-1);
    }
    APP_PRINT("[CPU0] Compare: MATCH\r\n");

    /* Close the driver. */
    err = R_SHARED_MEMORY_Close(&g_shared_memory0_ctrl);
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("[CPU0] Close failed: %d\r\n", err);
        APP_ERR_TRAP(err);
    }
    APP_PRINT("[CPU0] Close: done\r\n");
