...
void hal_entry(void)
{
    ...
    */
    ...
    /*---------- Sector Erase ----------*/
    uint8_t  * const erase_addr = (uint8_t *) QSPI_DEVICE_START_ADDRESS;
    uint32_t  const erase_len   = 4096U;
    APP_PRINT("[SECTOR ERASE] sector @0x%08X, Length %uB\r\n", erase_addr, (unsigned) erase_len);
    err = R_XSPI_QSPI_Erase(&g_qspi0_ctrl, erase_addr, erase_len);
    if (FSP_SUCCESS != err)
    {
...
void qspi_set_Quad_Mode_enable(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("[ENTER 4 BYTE ADDR MODE] DirectTransfer failed: err=%d\r\n", err);
        APP_ERR_TRAP(err);
    }
    else
    {
        APP_PRINT("[ENTER 4 BYTE ADDR MODE] sent: CMD=0x%02X\r\n", (unsigned) direct_command_EN4B.command);
    }
    /*--------------------------------------------------------------*/
