...
void hal_entry(void)
{
    ...
    APP_PRINT(MENUOPTION);

    /* Initialize BSC driver */
    err = bsc_init ();

    if (FSP_SUCCESS != err)
    {   
        /* BSC module init failed */
        APP_PRINT("\r\n ** BSC INIT FAILED ** \r\n");
        APP_ERR_TRAP(err);
    }
    ...
    while (EXIT != converted_rtt_input)
    {
        ...
        {
            APP_READ(rByte);
            converted_rtt_input = (uint8_t)atoi((char *)rByte);   /* Cast to char */
            ...
            switch (converted_rtt_input)
            {
                case ERASE:
                    ...
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("** BSC ERASE Demo failed **  \r\n");
                        bsc_deinit ();
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("** BSC ERASE Demo Successful**  \r\n");
                    }
                    break;
                case WRITE:
                    err = bsc_write ();
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("\r\n** BSC WRITE Demo failed **  \r\n");
                        bsc_deinit ();
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("** BSC WRITE Demo Successful**  \r\n");
                    }
                    break;
                case WRITE_BUFF:
                    err = bsc_write_buff ();
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("\r\n** BSC WRITE BUFFER Demo failed **  \r\n");
                        bsc_deinit ();
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("** BSC WRITE BUFFER Demo Successful**  \r\n");
                    }
                    break;
                case EXIT:
                    err = bsc_deinit ();
                    if (FSP_SUCCESS != err)
                    {
                        APP_PRINT("** BSC EXIT Demo failed ** \r\n");
                        APP_ERR_TRAP(err);
                    }
                    else
                    {
                        APP_PRINT("** BSC EXIT Demo Successful**  \r\n");
                    }
                    break;
                default:
            ...
            if ((ERASE <= converted_rtt_input) && (EXIT > converted_rtt_input))
            {
