...
typedef enum slave
{
    ...
    SLAVE_WRITE = 2U,
    SLAVE_NO_RW = 3U
}e_slave_transfer_mode_t;

/* Slave transmit buffer */
static uint8_t s_g_slave_tx_buf[BUF_LEN] __attribute__((section("UNCACHED_BSS")));

/* Slave receive buffer */
static uint8_t s_g_slave_rx_buf[BUF_LEN] __attribute__((section("UNCACHED_BSS")));

/* Master buffer for both read and write */
static uint8_t s_g_master_buf[BUF_LEN] __attribute__((section("UNCACHED_BSS")));

/* capture callback event for Slave and master module */
static volatile i2c_master_event_t s_g_master_event = (i2c_master_event_t)RESET_VALUE;
static volatile i2c_slave_event_t  s_g_slave_event  = (i2c_slave_event_t)RESET_VALUE;

/* Capture return value from slave read and write API */
static volatile fsp_err_t s_g_slave_api_ret_err = FSP_SUCCESS;

/* Event flag to identify the slave event.  */
static volatile e_slave_transfer_mode_t s_g_slave_RW  = SLAVE_READ;
...
fsp_err_t init_i2C_driver(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        if (R_IIC_MASTER_Close (&g_i2c_master_ctrl) != FSP_SUCCESS)
        {
            /* Display failure message in RTT */
    ...
    switch (s_g_slave_RW)
    {
        case SLAVE_READ:
        {
            ...
            else
            {
                ...
                /* print message */
                APP_PRINT ("\r\n** Slave read operation is successful **\r\n");
            }

            /* set variable value to perform slave write operation.*/
            s_g_slave_RW = SLAVE_WRITE;
        ...
        case SLAVE_WRITE:
        {
            ...
            else
            {
                ...
                /* print message */
                APP_PRINT ("\r\n** Slave Write operation is successful **\r\n");
            }

            /* set variable value to perform slave read operation.*/
            s_g_slave_RW = SLAVE_READ;
...
static fsp_err_t iic_slave_write(void)
{
    ...
    memset(s_g_master_buf, RESET_VALUE, BUF_LEN);
    memcpy(s_g_slave_tx_buf, write_buffer, BUF_LEN);

    /* resetting callback event */
    s_g_master_event = (i2c_master_event_t)RESET_VALUE;
    s_g_slave_event  = (i2c_slave_event_t)RESET_VALUE;

    /* Start master read.  Master has to initiate the transfer. */
    write_err = R_IIC_MASTER_Read(&g_i2c_master_ctrl, s_g_master_buf, BUF_LEN, false);
    ...
    while ((I2C_SLAVE_EVENT_TX_COMPLETE != s_g_slave_event) || (I2C_MASTER_EVENT_RX_COMPLETE != s_g_master_event))
    {
        /* check for aborted event */
        if ((I2C_MASTER_EVENT_ABORTED == s_g_master_event) || (I2C_SLAVE_EVENT_ABORTED == s_g_slave_event))
        {
        ...
        /*
         * handle error for slave write API return value s_g_slave_api_ret_err gets
         * updated only in case of error return from SLAVE write API
         */
        else if (FSP_SUCCESS != s_g_slave_api_ret_err)
        {
            write_err = s_g_slave_api_ret_err;

            /* reset this with success again for further usage to capture in case of API failure */
            s_g_slave_api_ret_err = FSP_SUCCESS;
        ...
        else
        {
            ...
            {
                /* Timeout reached: I2C event not occurred during slave write operation */
    ...
    if (BUFF_EQUAL == memcmp(s_g_slave_tx_buf, s_g_master_buf, BUF_LEN))
    {
...
static fsp_err_t iic_slave_read(void)
{
    ...
    memset(s_g_slave_rx_buf, RESET_VALUE, BUF_LEN);
    memcpy(s_g_master_buf, read_buffer, BUF_LEN);
    ...
    read_err = R_IIC_MASTER_Write(&g_i2c_master_ctrl, s_g_master_buf, BUF_LEN, false);
    ...
    while ((I2C_SLAVE_EVENT_RX_COMPLETE != s_g_slave_event) || (I2C_MASTER_EVENT_TX_COMPLETE != s_g_master_event))
    {
        /* check for aborted event */
        if ((I2C_MASTER_EVENT_ABORTED == s_g_master_event) || (I2C_SLAVE_EVENT_ABORTED == s_g_slave_event))
        {
        ...
        /*
         * handle error for slave read API return value s_g_slave_api_ret_err gets
         * updated only in case of error return from SLAVE read API
         */
        else if (FSP_SUCCESS != s_g_slave_api_ret_err)
        {
            read_err = s_g_slave_api_ret_err;
        ...
        else
        {
            ...
            {
                /* Timeout reached: I2C event not occurred during slave read operation */
    ...
    if (BUFF_EQUAL == memcmp(s_g_slave_rx_buf, s_g_master_buf, BUF_LEN))
    {
    ...
    if (NULL != p_args)
    {
        switch (p_args->event)
        {
            case I2C_SLAVE_EVENT_RX_REQUEST:
            {
                ...
                err = R_IIC_SLAVE_Read(&g_i2c_slave_ctrl, s_g_slave_rx_buf, BUF_LEN);
                if (FSP_SUCCESS != err)
                {
                    /* Update return error here */
                    s_g_slave_api_ret_err = err;
                }
            ...
            case I2C_SLAVE_EVENT_TX_REQUEST:
            {
                ...
                err = R_IIC_SLAVE_Write(&g_i2c_slave_ctrl, s_g_slave_tx_buf, BUF_LEN);
                if (FSP_SUCCESS != err)
                {
                    /* Update return error here */
                    s_g_slave_api_ret_err = err;
...
void set_led(bsp_io_level_t led_state)
{
    ...
    {
        for (uint8_t cnt = RESET_VALUE; cnt < leds.led_count; cnt++)
        {
            R_BSP_PinClear((bsp_io_region_t) (leds.p_leds[cnt][1]), (bsp_io_port_pin_t) (leds.p_leds[cnt][0]));
        }
    } else if (LED_ON == led_state)
    {
        for (uint8_t cnt = RESET_VALUE; cnt < leds.led_count; cnt++)
        {
            R_BSP_PinSet((bsp_io_region_t) (leds.p_leds[cnt][1]), (bsp_io_port_pin_t) (leds.p_leds[cnt][0]));
        }
    } else
    {
        ;   /* Do nothing */
 ...
 * @brief     Close the IIC Master and Slave module. Handle the Error internally with Proper Message.
