...
void hal_entry(void)
{
    ...
    /* Enable Start source for Timer in Periodic mode */
    ...
    while (true)
    {
        if (APP_CHECK_DATA)
        {
            ...
            /* Enter "HIGH" from P17_1(GPIO) to P16_7(ELC Single Port 0) to generate the event. */
            /* When P17_1 and P16_7 are connected on the board, the timer start counting
             * without calling timer start API. */
            R_BSP_PinAccessEnable();
            R_BSP_PinToggle(BSP_IO_REGION_SAFE, BSP_IO_PORT_18_PIN_2);
            ...
            err = R_GPT_StatusGet(&g_timer_ctrl, &status);
            if (FSP_SUCCESS != err)
            {
                elc_deinit();
