...
static void can_write_operation(canfd_instance_ctrl_t * p_api_ctrl, can_frame_t * p_frame)
{
    ...
    while ((true != b_canfd_ch0_tx_complete) && (true != b_canfd_ch1_tx_complete))
    {
        ...
        {
            APP_ERR_PRINT("\ncan_write_operation failed due to timeout");
            led_update(error);
            APP_ERR_TRAP(true);
            break;
        }
...
void can_read_operation(void)
{
    ...
    if (true == b_canfd_ch0_rx_complete)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("\n** R_CANFD_InfoGet API for channel 0 failed **");
            led_update(error);
            canfd_deinit();
            APP_ERR_TRAP(err);
        }
        ...
        if (can_ch0_rx_info.rx_mb_status)
        {
            ...
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("\n** R_CANFD_Read API for channel 0 failed **");
                led_update(error);
                canfd_deinit();
                APP_ERR_TRAP(err);
            }
    ...
    else if (true == b_canfd_ch1_rx_complete)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("\n** R_CANFD_InfoGet API for channel 1 failed **");
            led_update(error);
            canfd_deinit();
            APP_ERR_TRAP(err);
        }
        ...
        if (can_ch1_rx_info.rx_mb_status)
        {
            ...
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("\n** R_CANFD_Read API for channel 1 failed **");
                led_update(error);
                canfd_deinit();
                APP_ERR_TRAP(err);
            }
...
void canfd_deinit(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\n** R_CANFD_Close API for channel 0 failed **");
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\n** R_CANFD_Close API for channel 1 failed **");
    }
}
