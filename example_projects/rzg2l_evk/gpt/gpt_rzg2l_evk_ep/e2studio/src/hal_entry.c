        ...
        if (APP_CHECK_DATA)
        {
            ...
            switch (read_data)
            {
                case PERIODIC_MODE_TIMER:
                {
                    ...
                    p0clk_freq_hz >>= (uint32_t)(g_timer_periodic_cfg.source_div); /* Cast to 32-bit integer type */

                    /* Convert period to P0CLK counts so it can be set in hardware. */
                    period_counts = (uint64_t)((gpt_desired_period_ms * (p0clk_freq_hz * CLOCK_TYPE_SPECIFIER)) / 
