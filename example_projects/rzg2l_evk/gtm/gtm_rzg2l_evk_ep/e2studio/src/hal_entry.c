...
void hal_entry(void)
{
    ...
    timer_info_t  one_shot_info =
    {
        ...
        .count_direction = (timer_direction_t) RESET_VALUE,  /* Cast to match timer_direction_t */
    ...
    {
        gtm_deinit();
        APP_ERR_PRINT("\r\nR_GTM_InfoGet API failed. Closing all drivers. Restart the Application\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    raw_counts_oneshot = (uint32_t)(time_period_ms_oneshot * (clock_freq / 1000)); /* Cast to uint32_t type */
    ...
    {
        gtm_deinit();
        APP_ERR_PRINT("\r\nR_GTM_PeriodSet API failed. Closing all drivers. Restart the Application\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    raw_counts_periodic = (uint32_t)(time_period_ms_periodic * (clock_freq / 1000)); /* Cast to uint32_t type */
    ...
    while (true)
    {
        ...
        /* Check if GTM2 is enabled in one-shot mode */
        if (SET_FLAG == g_one_shot_timer_flag)
        {
            g_one_shot_timer_flag = RESET_FLAG;
            APP_PRINT("\r\nGTM2 is Enabled in one-shot mode");
        }
...
static fsp_err_t timer_status_check(void)
{
    ...
    {
        .counter = RESET_VALUE,
        .state = (timer_state_t) RESET_VALUE,  /* Cast to match timer_direction_t */
    };
