        ...
        if (APP_CHECK_DATA)
        {
            ...
            switch (read_data)
            {
                case PERIODIC_MODE_TIMER:
                {
                    ...
                    clk_freq_hz >>= (uint32_t)(g_timer_periodic_cfg.source_div);  /* cast to uint32_t */
    
                    /* Convert period to clk counts so it can be set in hardware. */
                    period_counts = (uint64_t)((gpt_desired_period_ms * (clk_freq_hz *CLOCK_TYPE_SPECIFIER))
                ...
                case PWM_MODE_TIMER:
                {
                    ...
                    else
                    {
                        ...
                        err = set_timer_duty_cycle((uint8_t)gpt_desired_duty_cycle_percent); /* cast to uint8_t */
                        if (FSP_SUCCESS != err)
                        {
