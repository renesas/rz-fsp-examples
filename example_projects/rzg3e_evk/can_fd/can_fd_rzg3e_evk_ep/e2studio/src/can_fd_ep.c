...
bool g_canfd_ch4_tx_complete = false;
bool g_canfd_ch4_rx_complete = false;
bool g_canfd_ch1_tx_complete = false;
bool g_canfd_ch1_rx_complete = false;
...
void canfd_operation(void)
{
    ...
    memcpy((uint8_t*)&g_canfd_ch4_tx_frame.data[ZERO], (uint8_t*)&g_tx_data[ZERO], CAN_FD_DATA_LENGTH_CODE);
...
/***********************************************************************************************************************
 * @brief       This function transmits data on classic CAN or CANFD frame.
 * @param[in]   p_api_ctrl          CANFD driver control block (instance control) used for transmission.
...
static void can_write_operation(canfd_instance_ctrl_t p_api_ctrl, can_frame_t can_transmit_frame)
{
    ...
    while ((true != g_canfd_ch4_tx_complete) && (true != g_canfd_ch1_tx_complete))
    {
        g_time_out--;
    ...
    /* Reset flag bit */
    if (true == g_canfd_ch4_tx_complete)
    {
        g_canfd_ch4_tx_complete = false;
    }
    if (true == g_canfd_ch1_tx_complete)
    {
        g_canfd_ch1_tx_complete = false;
    }
}

/***********************************************************************************************************************
 * @brief       This function compares transmitted/received data and to send ACK accordingly.
 * @param[in]   None.
 * @return      None.
 **********************************************************************************************************************/
static void can_data_check_operation(void)
{
    ...
    if (RESET_VALUE == strncmp((char*)&g_canfd_ch1_rx_frame.data[ZERO], \
                            (char*)&g_tx_data[ZERO], CAN_CLASSIC_FRAME_DATA_BYTES))
    {
    ...
    else if (RESET_VALUE == strncmp((char*)&g_canfd_ch4_rx_frame.data[ZERO],
                                    (char*)&g_rx_data[ZERO], CAN_CLASSIC_FRAME_DATA_BYTES))
    {
        /* Clean receive frame */
        ...
        /* Update FD frame parameters for channel 4 */
        g_canfd_ch4_tx_frame.id = CAN_ID;
        g_canfd_ch4_tx_frame.id_mode = CAN_ID_MODE_EXTENDED;
    ...
    /* Acknowledge for second transmission */
...
/***********************************************************************************************************************
 * @brief       This function reads channel status info and read data.
 * @param[in]   None.
 * @return      None.
 **********************************************************************************************************************/
void can_read_operation(void)
{
    ...
    if (true == g_canfd_ch4_rx_complete)
    {
        /* Get the status information for CAN transmission */
        err = R_CANFD_InfoGet(&g_canfd_ch4_ctrl, &g_can_ch4_rx_info);
        ...
        if ((g_can_ch4_rx_info.rx_mb_status[0]) || (g_can_ch4_rx_info.rx_mb_status[1]) || \
                (g_can_ch4_rx_info.rx_mb_status[2]))
        {
    ...
    else if (true == g_canfd_ch1_rx_complete)
    {
        /* Get the status information for CAN transmission */
        err = R_CANFD_InfoGet(&g_canfd_ch1_ctrl, &g_can_ch1_rx_info);
        ...
        /* Check if the data is received in FIFO */
        if ((g_can_ch1_rx_info.rx_mb_status[0]) || (g_can_ch1_rx_info.rx_mb_status[1]) || \
                (g_can_ch1_rx_info.rx_mb_status[2]))
        {
            /* Read the input frame received */
...
/***********************************************************************************************************************
 * @brief       This function updates data buffer that is to be compared with transmitted FD frame data.
 * @param[in]   None.
 * @return      None.
 **********************************************************************************************************************/
static void can_fd_data_update(void)
...
void led_update(led_state_t led_state)
{
    switch (led_state)
    {
        case error:
        {
            ...
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD0], BSP_IO_LEVEL_HIGH);
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD1], BSP_IO_LEVEL_HIGH);
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD2], BSP_IO_LEVEL_HIGH);
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD3], BSP_IO_LEVEL_HIGH);
            break;
        }
        case successful:
        {
            ...
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD0], s_pin_level);
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD1], s_pin_level);
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD2], s_pin_level);
            R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t)g_leds.p_leds[LED_LD3], s_pin_level);
...
/***********************************************************************************************************************
 * @brief      This is CANFD callback function.
 * @param[in]  p_args.
 * @retval     None.
 **********************************************************************************************************************/
void canfd_ch4_callback(can_callback_args_t *p_args)
{
    switch (p_args->event)
    {
        ...
        {
            g_canfd_ch4_tx_complete = true;     /* Set flag bit */
            break;
        }
        case CAN_EVENT_RX_COMPLETE:
        {
            g_canfd_ch4_rx_complete = true;     /* Set flag bit */
            break;
        }
...
void canfd_ch1_callback(can_callback_args_t *p_args)
{
    switch (p_args->event)
    {
        case CAN_EVENT_TX_COMPLETE:
        {
            g_canfd_ch1_tx_complete = true;     /* Set flag bit */
            break;
        }
        case CAN_EVENT_RX_COMPLETE:
        {
            g_canfd_ch1_rx_complete = true;     /* Set flag bit */
            break;
        }
        case CAN_EVENT_ERR_WARNING:             /* Error warning event */
        case CAN_EVENT_ERR_PASSIVE:             /* Error passive event */
...
/***********************************************************************************************************************
 * @brief       This function de-initializes the CANFD module.
 * @param[in]   None.
 * @return      None.
 **********************************************************************************************************************/
