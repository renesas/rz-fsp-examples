...
#define EP_INFO     "\r\nThis example project demonstrates the typical use of the WDT HAL module APIs in collaboration"\
                    "\r\nwith the GTM timer interrupt and the PMOD LED/Button."\
                    "\r\nOn user input, the project initializes the WDT (timeout configured to 0.175 seconds) and"\
                    "\r\nstarts a GTM timer that expires every 50 milliseconds."\
                    "\r\nEach GTM expiry refreshes the WDT counter and toggles LD0 on the PMOD LED." \
                    "\r\nThe WDT refresh happens every 50 milliseconds, but the counter refresh status is printed"\
                    "\r\non the J-Link RTT Viewer once every 3 seconds."\
