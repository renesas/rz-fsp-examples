...
1. Timer period range in millisecond as per Example Project configuration:
   RZ/G2LC      --      (0 to 42949) ms
2. User is expected to enter data of size not exceeding 15 bytes.
   Operation is not guaranteed for any user input value other than integer, char (i.e. float, special char)
   through RTT.
   (decimal part and/or non-numeric characters are ignored), which may lead to unexpected results.
