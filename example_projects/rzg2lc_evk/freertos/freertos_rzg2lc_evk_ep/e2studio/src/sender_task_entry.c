...
/***********************************************************************************************************************
 * @brief        This function initializes GTM timer and periodically send messages to the queue; on suspend request,
 *               stops/closes the timer, suspends the receiver task, resumes the Semaphore task, then suspends self.
...
void sender_task_entry(void *pvParameters)
{
    ...
    /* Start GTM timer to start sending message to Message Queue periodically at 1 second from GTM callback */
    ...
    while (true)
    {
         ...
         * continue sending messages to Message Queue at 500 milliseconds periodically. */
        if (true != b_suspend_msqQ_task)
        {
            ...
            if (pdPASS == xQueueSendToBack(g_queue, &s_message_to_task, Queue_wait_ticks))
            {
                ...
                APP_PRINT("\r\nSender_Task : Going on delay for 500 milliseconds");

                /* Places the task into the Blocked state. The task
                 * will execute every 500 milliseconds once */
                vTaskDelayUntil(&last_execution_time, WAIT_TIME);

                APP_PRINT("\r\nSender_Task : After delay of 500 milliseconds\r\n");
            }
        }
        else
        {
            ...
            if (pdPASS == xQueueReset(g_queue))
            {
                ...
                if (FSP_SUCCESS != fsp_err)
                {
                    if (FSP_SUCCESS != R_GTM_Close(&g_periodic_timer_msgq_ctrl))
                    {
                        ...
                        APP_ERR_PRINT("\r\nSender_Task : R_GTM_Close API failed\r\n");
                    }

                    /* Print out in case of error */
                    APP_ERR_PRINT("\r\nSender_Task : R_GTM_Stop API failed\r\n");
                    APP_ERR_TRAP(fsp_err);
                }
...
/***********************************************************************************************************************
 * @brief       This is user defined GTM IRQ callback for Message Queue.
 * @param[in]   p_args.
 * @return      None.
 **********************************************************************************************************************/
void periodic_timer_msgq_cb(timer_callback_args_t *p_args)
{
    ...
    /* Initialize the message to send to Message Queue */
    static st_msg_t s_message_to_task = {200, "GTM Callback"};
