...
fsp_err_t spi_init(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        APP_ERR_PRINT("** R_SPI_B_Open API failed **\r\n");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        if ((FSP_SUCCESS != R_SPI_B_Close(&g_spi_master_ctrl)))
        {
            ...
            APP_ERR_PRINT("** R_SPI_B_Close API failed **\r\n");
        }
...
/***********************************************************************************************************************
 * @brief       This function writes data to the MX25L12835F SPI flash memory by erasing a sector and then 
 *              programming a page.
 * @param[IN]   None.
...
fsp_err_t spi_write(void)
{
    ...
    /* Erase a sector on the MX25L12835F */
    ...
    /* Write a page to the MX25L12835F */
...
/***********************************************************************************************************************
 * @brief       This function demos R_SPI_B_Read().
 * @param[IN]   None.
 * @retval      FSP_SUCCESS                  Upon successful SPI Read.
 * @retval      Any other error code apart from FSP_SUCCESS  Unsuccessful Read.
 **********************************************************************************************************************/
...
/***********************************************************************************************************************
 * @brief       Assert the Chip Select (CS) pin for the MX25L12835F flash memory.
 *              Sets the pin to a low level and introduces a short delay for signal stabilization.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void mx25l1235f_cs_assert()
...
/***********************************************************************************************************************
 * @brief       Deassert the Chip Select (CS) pin for the MX25L12835F flash memory.
 *              Sets the pin to a high level and introduces a short delay for signal stabilization.
 * @param[in]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void mx25l1235f_cs_deassert()
...
/***********************************************************************************************************************
 * @brief       Implement the MX25L1235F SPI bulk transfer interface.
 *              Handles transmission, reception, or simultaneous write/read operations.
 * @param[IN]   tx_data              Pointer to the transmit data buffer (can be NULL).
 * @param[OUT]  rx_data              Pointer to the receive data buffer (can be NULL).
 * @param[IN]   length               Number of bytes to transfer.
 * @retval      MX25L12835F_SUCCESS  Data transfer completed successfully.
 * @retval      MX25L12835F_ERROR    Transfer failed or timeout occurred.
 **********************************************************************************************************************/
static mx25l1235f_status_t mx25l1235f_spi_transfer_bulk(uint8_t const *tx_data, uint8_t *rx_data, uint32_t length)
{
