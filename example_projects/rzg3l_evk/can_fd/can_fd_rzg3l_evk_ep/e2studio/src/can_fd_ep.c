...
static void can_write_operation(canfd_instance_ctrl_t * p_api_ctrl, can_frame_t * p_frame)
{
    ...
    /* Transmit the data from mail box #0 with TX frame */
    err = R_CANFD_Write(p_api_ctrl, CAN_MAILBOX_NUMBER_0, p_frame);

    /* Handle error */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** R_CANFD_Write API failed **\r\n");
        led_update(error);
        canfd_deinit();
        APP_ERR_TRAP(err);
    }
    ...
    while ((true != b_canfd_ch0_tx_complete) && (true != b_canfd_ch1_tx_complete))
    {
        ...
        {
            APP_ERR_PRINT("** can_write_operation failed due to timeout **\r\n");
            led_update(error);
            APP_ERR_TRAP(true);
            break;
        }
...
static void can_data_check_operation(void)
{
    ...
    {
        /* Clean receive frame - clear actual received data length */
        memset(&g_canfd_ch1_rx_frame.data[ZERO], NULL_CHAR, g_canfd_ch1_rx_frame.data_length_code);
    ...
    {
        /* Clean receive frame - clear actual received data length */
        memset(&g_canfd_ch0_rx_frame.data[ZERO], NULL_CHAR, g_canfd_ch0_rx_frame.data_length_code);
    ...
    {
        /* Clean receive frame - clear entire FD frame buffer */
        memset(&g_canfd_ch1_rx_frame.data[ZERO], NULL_CHAR, SIZE_64);
    ...
    {
        /* Clean receive frame - clear entire FD frame buffer */
        memset(&g_canfd_ch0_rx_frame.data[ZERO], NULL_CHAR, SIZE_64);
    ...
    {
        APP_ERR_PRINT("\n** CAN data mismatch **\r\n** can_data_check_operation for channel 0 failed **\r\n");
        led_update(error);
        APP_ERR_TRAP(true);
    }
}
...
void can_read_operation(void)
{
    ...
    if (true == b_canfd_ch0_rx_complete)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("** R_CANFD_InfoGet API for channel 0 failed **\r\n");
            led_update(error);
            canfd_deinit();
            APP_ERR_TRAP(err);
        }
        ...
        if (can_ch0_rx_info.rx_mb_status[0])
        {
            ...
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("** R_CANFD_Read API for channel 0 failed **\r\n");
                led_update(error);
                canfd_deinit();
                APP_ERR_TRAP(err);
            }
    ...
    else if (true == b_canfd_ch1_rx_complete)
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("** R_CANFD_InfoGet API for channel 1 failed **\r\n");
            led_update(error);
            canfd_deinit();
            APP_ERR_TRAP(err);
        }
        ...
        /* Check if the data is received in FIFO */
        if (can_ch1_rx_info.rx_mb_status[0])
        {
            /* Read the input frame received */
            ...
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("** R_CANFD_Read API for channel 1 failed **\r\n");
                led_update(error);
                canfd_deinit();
                APP_ERR_TRAP(err);
            }
