...
1. Project Overview:
    ...
    The Port Output can be disabled using the below three options:
    	1. Using POEG trigger pin
           This mode supports disabling GPT output pins based on GTETRG input pin level.
    	2. Using POEG software API
           This mode supports disabling GPT output pins by R_POEG_OutputDisable() software API request.
    	3. Using POEG trigger GPT output level
           This mode supports disabling GPT output pin using POEG trigger GPT output level.
    	4. Reset the POEG status
           This option resets the POEG0 and POEG3 status.
    The Port Output can be enabled using the POEG Reset API.
    The status of each mode is displayed on J-Link RTT Viewer.
...
5. Hardware Connections:
    ...
    (3) Connect PMOD LED to PMOD1_6A (PIN7-12) on the SMARC Carrier Board for LED output control: 
    	- LD2 for POEG 0
    	- LD3 for POEG 3
    (4) Connect PMOD0_2A PIN10 to PMOD0_2A PIN3 on SMARC Carrier Board using jumper wire.

Note:
1. Segger RTT block address may be needed to download and observe EP operation using a hex file with RTT-Viewer.
   RTT Block address for hex file committed in repository are as follows:
   a. e2studio: 0x000285f8
...
3. User can monitor the GPT signals using a logic analyzer to observe the effects of POEG output disable and
   POEG reset operations : 
      GPT5 (used for POEG trigger pin and POEG software API modes): PMOD0_2A Pin 2 and PMOD0_2A Pin 4.
      GPT0 (used for POEG trigger GPT output level mode): PMOD1_6A Pin 3 and PMOD1_6A Pin 4.
