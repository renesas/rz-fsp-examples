 ...
 *  @param[in]   None.
 *  @retval      FSP_SUCCESS     Upon successful demonstration.
 *  @retval      Any other error code apart from FSP_SUCCESS     Unsuccessful demonstration.
 **********************************************************************************************************************/
fsp_err_t uart_ep_demo(void)
{
    ...
    while (true)
    {
        if (s_data_received_flag)
        {
            ...
            input_length = ((uint8_t)(strlen((char *) &s_temp_buffer)));  /* Cast to uint8_t */
            ...
            if (DATA_LENGTH > (uint8_t)input_length)
            {
                ...
                cycle = ((uint32_t)(atoi((char *) &s_temp_buffer)));   /* Cast to uint32_t */
 ...
 *  @param[in]   p_msg      Pointer to user input message.
...
void user_uart_callback(uart_callback_args_t *p_args)
{
    ...
    /* Reset s_counter_var index if it exceeds than buffer size */
