...
static void process_sd_operation(uint8_t p_input_buffer);     /* Process SD operations */
...
__attribute__((section("UNCACHED_BSS"), aligned(64))) static uint8_t s_write_data[BUFFER_SIZE];
__attribute__((section("UNCACHED_BSS"), aligned(64))) static uint8_t s_read_data[BUFFER_SIZE];

/***********************************************************************************************************************
 * @brief       This function initializes SDHI with FreeRTOS+FAT and handles SD card operations
 *              and events in a continuous loop.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
void sdhi_thread_entry(void *pvParameters)
{
    ...
    while (true)
    {
        uint8_t input_buf[BUFFER_SIZE_DOWN] = {RESET_VALUE};
        uint8_t converted_input             = RESET_VALUE;

        /* Read Terminal Emulator input from user */
        if (APP_CHECK_DATA)
        {
            APP_READ (input_buf);
            converted_input = (uint8_t)atoi((char *)input_buf);
            process_sd_operation (converted_input);
        }

        /* Detect SD card removal event */
        xEventGroupValue = xEventGroupGetBitsFromISR(g_SDHI_EventGroupHandle);

        if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED == xEventGroupValue)
        {
            /* Clear event after handling */
            xEventGroupClearBits(g_SDHI_EventGroupHandle, RM_BLOCK_MEDIA_EVENT_MEDIA_REMOVED);

            APP_ERR_PRINT("\r\n SD Card disconnected without Eject option\r\n");
            APP_ERR_PRINT("\r\n Connect the SD Card and Execute Safely Eject option to make sure "
            ...
            xEventGroupValue = xEventGroupWaitBits(g_SDHI_EventGroupHandle,
                                                    ...
                                                    pdFALSE,
                                                    portMAX_DELAY);
            APP_PRINT("\r\nSD card inserted, please enter 3 for Executing Safely Eject\r\n");
        }
    }
}
 ...
 * @param[IN]   input_buffer    Converted input.
 * @retval      None.
 **********************************************************************************************************************/
...
static void sd_write_operation(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        p_file_pointer = ff_fopen((const char *)FILE_NAME, WRITE_MODE);
        if (NULL != p_file_pointer)
        {
            ...
            /* Write API writes */
            bytes_written = ff_fwrite (s_write_data, sizeof(s_write_data[RESET_VALUE]), BUFFER_SIZE, p_file_pointer);
            ...
            if (FF_ERR_NONE == file_error)
            {
                ...
                {
                    APP_PRINT("    %zu bytes data successfully written to file %s\r\n", bytes_written, FILE_NAME);
...
static void sd_read_operation(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        p_file_pointer = ff_fopen((const char *)FILE_NAME, READ_MODE);
        if (NULL != p_file_pointer)
        {
            ...
            bytes_read = ff_fread (s_read_data, sizeof(s_read_data[RESET_VALUE]), BUFFER_SIZE, p_file_pointer);
            ...
            if (FF_ERR_NONE == file_error)
            {
                ...
                {
                    APP_PRINT("    %zu bytes Data successfully read from file %s\r\n", bytes_read, FILE_NAME);
                ...
                if (FF_ERR_NONE == memcmp(s_write_data, s_read_data, BUFFER_SIZE))
                {
...
static void format_sd_device(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        {
            char msg[128];
            snprintf(msg, sizeof(msg), "\r\n FF_Partition API failed  %s. Check the SD Card Device.\r\n",
                    FF_GetErrMessage(ff_err));
            APP_ERR_PRINT(msg);
            APP_PRINT(" %s\r\n", FF_GetErrMessage(ff_err));
        }

        /* Format time varies with SD Card capacity. Might take longer time for higher capacity SD Card */
        ff_err = FF_Format (&my_disk, my_disk.xStatus.bPartitionNumber, pdFALSE, pdFALSE);
        if (FF_ERR_NONE != ff_err)
        {
            char msg[128];
            snprintf(msg, sizeof(msg), "\r\n FF_Format API failed  %s. Check the SD Card.\r\n",
                    FF_GetErrMessage(ff_err));
            APP_ERR_PRINT(msg);
            APP_PRINT(" %s\r\n", FF_GetErrMessage(ff_err));
        }
...
static void update_buffer(void)
{
    ...
    {
        s_write_data[buf_index] = ASCII_CHAR_A;
        s_read_data[buf_index] = RESET_VALUE;
    }
}
 ...
 * @retval      None.
 **********************************************************************************************************************/
void freertos_fat_callback(rm_freertos_plus_fat_callback_args_t *p_args)
{
...
static void sd_display_cwd(void)
{
    ...
    if (RM_FREERTOS_PLUS_FAT_EVENT_MEDIA_REMOVED != xEventGroupValue)
    {
        ...
        {
            APP_PRINT("\r\nDirectory rza_dir is created in root directory\r\n\r\n");
        }
