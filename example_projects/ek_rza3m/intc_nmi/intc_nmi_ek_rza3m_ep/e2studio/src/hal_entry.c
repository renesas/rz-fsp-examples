...
void hal_entry(void)
{
    ...
    bsp_io_level_t led_current_state = (bsp_io_level_t) RESET_VALUE; /* Cast to bsp_io_level_t */
    ...
    /* Initialize NMI driver */
    err = intc_nmi_init();

    /* Handle error */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** intc_nmi_init failed **\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** intc_nmi_enable failed **\r\n");
    ...
    while (true)
    {
        ...
        if (true == g_sw_press)
        {
            ...
            /* Notify that the user pushbutton has been pressed */
            APP_PRINT("User Pushbutton Pressed\r\n");

            /* Get pin to toggle */
            bsp_io_port_pin_t pin = g_bsp_leds.p_leds[0];
            ...
            /* Write to this pin */
            err = R_IOPORT_PinWrite(&g_ioport_ctrl, pin, led_current_state);
