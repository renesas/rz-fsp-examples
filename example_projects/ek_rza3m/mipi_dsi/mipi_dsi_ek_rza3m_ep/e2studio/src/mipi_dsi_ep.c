...
uint32_t g_read_data      = RESET_VALUE;
uint16_t g_period_sec     = RESET_VALUE;

timer_info_t g_timer_info = {.clock_frequency = RESET_VALUE,\
...
void mipi_dsi_push_table(const lcd_table_setting_t * table)
{
    ...
    while (MIPI_DSI_DISPLAY_CONFIG_DATA_END_OF_TABLE != p_entry->cmd_id)
    {
        ...
        if ((mipi_cmd_id_t) MIPI_DSI_DISPLAY_CONFIG_DATA_DELAY_FLAG == msg.cmd_id)
        {
...
/***********************************************************************************************************************
 * @brief      This function starts video mode and draws color bands on the LCD screen.
 * @param[in]  None.
 * @retval     None.
 **********************************************************************************************************************/
void mipi_dsi_start_display(void)
{
    ...
    err = R_GTM_InfoGet (&g_timer0_ctrl, &g_timer_info);
    ...
    while (true)
    {
        ...
        if (g_irq_state)
        {
            ...
            memset(g_touch_coordinates, 0, sizeof(g_touch_coordinates));

            err = gt911_get_status (&statusRegister, &g_touch_coordinates[0],
                                    (sizeof(g_touch_coordinates)) / (sizeof(g_touch_coordinates[0])));
    ...
    if (APP_CHECK_DATA)
        {
            ...
            g_read_data = process_input_data();
            switch (g_read_data)
            {
                ...
                case SELECT_5S:
                {
                    ...
                    g_period_sec = GTM_DESIRED_PERIOD_5SEC;
                    valid_input = true;
                    break;
                }
                ...
                case SELECT_15S:
                {
                    ...
                    g_period_sec = GTM_DESIRED_PERIOD_15SEC;
                    valid_input = true;
                    break;
                }
                ...
                case SELECT_30S:
                {
                    ...
                    g_period_sec = GTM_DESIRED_PERIOD_30SEC;
                    valid_input = true;
                    break;
                }
            ...
            if (valid_input)
            {
                /* Calculate the desired period based on the current clock.
                 * Note that this calculation could overflow if the
                 * desired period is larger than UINT32_MAX / pclkd_freq_hz.
                 * A cast to uint64_t is used to prevent this. */
                uint32_t period_counts = (uint32_t) (((uint64_t) g_timer_info.clock_frequency * g_period_sec)\
                                                     / GTM_UNITS_SECONDS);
...
/***********************************************************************************************************************
 * @brief      User defined function to draw the current display to a frame buffer.
 * @param[in]  None.
 * @retval     None.
 **********************************************************************************************************************/
static void display_draw(void)
{
    ...
    p_buff1 = (uint16_t *)fb_background[0]; /* Cast to uint16_t* */
...
/***********************************************************************************************************************
 *  @brief      This function processes an input string into an integer value.
 *  @param[in]  None.
...
static void mipi_dsi_ulps_exit(void)
{
    ...
    APP_PRINT("\r\nExited Ultra-low Power State (ULPS) due to touch with co-ordinates x: %u, ; y: %u.\r\n",
                g_touch_coordinates[0].x, g_touch_coordinates[0].y);
...
void handle_error(fsp_err_t err,  const char * err_str)
{
    if (FSP_SUCCESS != err)
    {
        ...
        {
            if (R_GTM_Close (&g_timer0_ctrl) != FSP_SUCCESS)
            {
        ...
        {
            if (R_LCDC_Close (&g_display_ctrl) != FSP_SUCCESS)
            {
        ...
        {
            if (R_INTC_IRQ_ExternalIrqClose (&g_external_irq_ctrl) != FSP_SUCCESS)
            {
        ...
        {
            if (R_RIIC_MASTER_Close (&g_i2c_master_ctrl) != FSP_SUCCESS)
            {
