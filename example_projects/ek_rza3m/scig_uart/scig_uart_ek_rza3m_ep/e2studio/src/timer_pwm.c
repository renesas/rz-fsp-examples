...
fsp_err_t set_cycle(uint32_t raw_count)
{
    ...
    else
    {
        ...
        g_raw_counts_periodic = (uint32_t)(raw_count *(g_clock_freq / 1000));

        /* Period Set API set the desired cycle on-board LED */
...
void periodic_timer_callback(timer_callback_args_t *p_args)
{
    ...
    /* Change LED state */
    fsp_err_t err = R_IOPORT_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t) g_bsp_leds.p_leds[1], led_level);
