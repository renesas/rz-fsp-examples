...
void reconfigure_transfer_settings(uint8_t * p_src, uint8_t * p_dest, transfer_mode_t e_mode)
{
    ...
    {
        transfer_info_recfg.p_src  = (void *) &p_src[0];    /* source address */
        transfer_info_recfg.p_dest = (void *) &p_dest[0];   /* destination address */
...
/***********************************************************************************************************************
 *  @brief       Print DMAC data transfered by g_transfer_write_ioport on User input
 *  @param[IN]   None
 *  @retval      None
 **********************************************************************************************************************/
...
void timer_callback(timer_callback_args_t * p_args)
{
    if (TIMER_EVENT_CYCLE_END == p_args->event)
    {
        ...
        for (uint32_t i = 0; i < leds.led_count; i++)
        {
            ...
            R_BSP_PinToggle((bsp_io_region_t)leds.p_leds[i][1], (bsp_io_port_pin_t)leds.p_leds[i][0]); /* LED toggle */
        }
    }
}
