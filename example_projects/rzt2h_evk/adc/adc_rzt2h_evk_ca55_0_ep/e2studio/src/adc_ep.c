...
/* Callback function */
void adc_callback (adc_callback_args_t * p_arg);

/***********************************************************************************************************************
 * @brief       This function reads the command (input) from TeraTerm and process the command(input).
 * @param[IN]   None
...
static fsp_err_t adc_scan_start(void)
{
    ...
    if (false == g_ready_to_read)
    {
        ...
        /* Disable interrupts */
        R_BSP_IrqDisable(g_adc_ctrl.p_cfg->scan_end_irq);
...
fsp_err_t adc_read_data(void)
{
    ...
    if ((ADC_MODE_SINGLE_SCAN == g_adc_cfg.mode) || (true == g_window_comp_event))
    {
        ...
        {
            /* Print voltage status warning to Tera Term*/
            ...
            else if (ADC_MODE_CONTINUOUS_SCAN == g_adc_cfg.mode)
            {
                adc_scan_start();
