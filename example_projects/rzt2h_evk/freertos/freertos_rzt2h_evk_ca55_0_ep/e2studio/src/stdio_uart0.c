...
int  app_check_data ();
int  app_printf (const char *p_format, ...);
/******************************************************************************
...
/**********************************************************************************************************************
 * Function Name: app_printf
 * Description  : Format a variable argument list into a temporary buffer and write the formatted data through
 *                stdio_write(). The data length is limited by the internal buffer size (650 bytes). This function
 *                acts similar to printf(), intended for printing logs from multiple threads. The caller must ensure
 *                thread-safe protection around stdio_write() if required.
 * Argument     : p_format   - Pointer to C-style format string
 * Return Value : Number of formatted characters written (or a negative value if formatting fails)
 *********************************************************************************************************************/
int app_printf(const char *p_format, ...)
{
    char    buff[700]; /* Temporary local buffer for formatted output */
    va_list args; /* Argument list handler */

    /* Initialize argument list from format string */
    va_start(args, p_format);

    /* Format output into buffer using safe bounded function */
    int len = vsnprintf(buff, sizeof(buff), p_format, args);
    ...
    if (len > 0)
    {
        stdio_write((uint8_t*)buff, (uint32_t)len);
    }

    return len;
}
/**********************************************************************************************************************
 End of function app_printf
 *********************************************************************************************************************/
